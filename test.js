...
4. По возможности использовать дефолт
  ...
  если после создание переменной firstComment и поставить точку при вызове, то можно увидеть все методы и свойства доступные для использования 
   на уровне объекта firstComment - собственные методы и методы на уровне объекта 

   firstComment -> Comment -> Object

   firstComment  является экземпляром коментария, коментарий в свою очередь содержит все методы объекта, который является глобальным классом в js -
   - это и есть цепочка прототипов в js

   
   ПРОВЕРКА ПРИНАДЛЕЖНОСТИ КОНКРЕТНОГО ЭКЗЕМПЛЯРА ТОМУ ИЛИ ИНОМУ КЛАССУ
   ...
   Когда мы создаем экземпляр класса коментарий, используя синтаксис new, а потом используем instanceof мы увидим true в данном случае, потому что
   firstComment является экземпляром коментария и наследует все методы класса Comment, и так же firstComment является экземпляром класса Object

   кроме этого можно вызывать метод upvote который находится на уровне коментария в классе для каждого экземпляра такого класса 
   ...
   используя метод hasOwnProperty, а этот метод наследуется от объекта и спомощью такого метода можно проверить принадлежность такого метода экземпляру
   конкретного класса т. е здесь мы проеряем есть ли у объекта firstComment собственное свойство которое называется text, а такое свойство есть.
   таким же способом мы можем проверить принадлежность свойства votesQty объекту firstComment.
   Но если мы выполним проверку свойства upvote, то мы увидим false потому что upvote это не собственный метод объекта firstComment - этот метод 
   наследуется от родительского класса Comment.
   Так же мы можем проверить принадлежность свойства hasOwnProperty - это метод который доступен на уровне объекта, здесь получим false потому что это
   не собственное свойство объекта firstComment
   ...
   const firstComment = new Comment('first comment')
   const firstComment = new Comment('second comment')
   const firstComment = new Comment('third comment')
 ...
 Кроме использования методов на уровне экземпляров можно в классе создавать статические методы. Такие методы создаются используя ключевое слово static
 оно стоит перед названием метода. У этого метода есть два параметра first и second, для простоты это просто строки и такой метод как mergeComments
 вернет две строки, объединив их с помощью шаблонной строки т.е в итоге мы хотим получить конкотенацию двух текстов и соединить их в один коментарий и
 для этого нам необходимо создать метод. Здесь нет необходимости создавать метод, который будет наследоваться всеми экземплярами класса Comment, 
 потому что наша задача просто объединить два текста , но эта задача касается коментариев, потому мы можем в данном примере создать статический метод
 и такой метод будет доступен как свойство класса comment то есть мы сможем вызвать его таким образом   
 Comment.mergeComments('First comment', 'Second comment')
 Здесь мы не создаем новый экземпляр класса Comment, здесь мы не используем ключевое слово new мы просто вызываем mergeComments как метод коментария -
 похожим образом мы поступали, когда вызывали такие методы как Object.assign, Object.keys и Object.values (является статическими методами объекта)

Методы, обозначенные ключевым словом static не наследуются экземплярами класса, но они доступны как методы конкретного класса, вызвать такой метод мы
 можем, используя точечную запись и в итоге мы получим объединение двух строк
    ...
    предположим что в нашей программе нужно послать зарос на удаленный сервер и получить ответ, мы можем послать запрос по сети через интернет, но
    мы не знаем когда сервер нам ответит(может ответить через 100 миллисекунд, 2 секунды, 5 секунд) т.е от нас это не зависит и не завсит от нашего 
    приложения, но при этом наше приложение должно ожидать ответа от сервера - ВОПРОС:  что делать нашемму приложению пока оно ожидает ответа от
    удаленного сервера?

    варианта два: наше приложение можент выполнять какие то другие действия пока ожидает, либо не выполнять ничего другого, так вот второй вариант 
    не желателен потому, что если это фронтэнд приложение и в этом приложении пользователи могут взаимодействовать со страницами(нажимать кнопки, 
    заполнять формы ), то пока мы ждем ответа от сервера, мы не свожем обрабатывать никакие действия пользователя(не сможем на них реагировать  с
    помощью js, потеряется интерактивность пользования сайтом ), потому мы должны иметь возможность выполнять другие действия пока мы ожидаем 
    ответа от сервера и аот такой запрос, который мы послали на сервер, называется АСИНХРОННЫМ ЗАПРОСО т.е мы не знаем когда мы получим ответ -
    ответ прихожит асинхронно т.е не сразу, а через какое то время и в этот интервал мы должны иметь возможность заниматься другими делами(выполнять
    другие задачи в нашем js  приложении) так вот в js с помощью промисов можно обрабатывать такие отложенные во времени события 

    ПРОМИС - ОБЕЩАНИЕ ПРЕДОСТАВИТЬ РЕЗУЛЬТАТ ПОЗЖЕ 

  промис сожет вернуть ошибку, если результат предоставить невозможно 

  СОСТОЯНИЕ ПРОМИСА 

  у промиса есть три состояния:

  1. Ожидание - когда промис только создается
  2. Исполнен - когда промис вернул результат (массив, объект и тд )
  3. Отклонен - если возникла ошшибка 


  СОЗДАНИЕ ПРОМИСА (промис - это объект)
  ...
  вновь созданный промис будет в состоянии ожидания(panding)

  в данном примере мы создаем промис с ключевым словом  new т.е мы создаем новый экземпляр класса Promise(такой класс уже есть в js и его не нужно 
    создавать)

    такой промис будет создан и будет присвоен переменной myPromise, которую мы объявили с помощью ключевого слова const. Так вот, конструктору, 
    который вызувается при создании нового экземпляра Promise мы передем колбэк функицию(в нашем примере стрелочная функиция)
    У этой колбэк функиции обязательно должно быть два параметра resolve и reject и внутри этой функиции происходит выполнение асинхронных действий,
    например мы посылаем запрос на удаленный сервер и внутри этой колбэк функиции нужно в результате вызвать одну из функций resolve ил reject, а это
    как раз функции и мы должны вызвать одну из них внуьтри блока(внутри функции Promise). И что происходит? - Когда мы вызываем resolve  мы должны
    передать какой то результат( какие то данные) и когда вызвана функиция resolve - промис считается исполненым, если же где то возникла ошибка. мы
    должны вызвать функицию reject и передать ей ту ошибку, которая возникла - в таком случае промис считается отклоненным

    Как же после создания промиса мы можем получить результат промиса?
    ...
    myPromise
      ...
      .catch(error => {
        ...
        // значение error - это значение, переданноуе в вызове функции reject внутри промиса
      })

 получение результата промиса происходит следующим образом - у объекта myPromise, который был создан на предыдцщем шаге, будет доступны такие методы как
      then и catch и эти методы так же можно комбинировать и в данном случае, когда мы вызываем метод then мы должны передать в качестве аргумента 
   также функцию и в данном случае это стрелочная функция с одним параметром .then(value =>..., так вот в этой стрелочной функции мы можем выполнять 
  действия в случае успешного исполнения промиса и значение параметра value  будет ровняться тому значеию, которое было передано функции resolve внутри 
  промиса.
  Во втором блоке, в вызове метода catch мы видим в качестве параметра ошибку и здесь нам следует выполнять действие на случай отклонения промиса, если
  внутри просмиса была вызвана функция reject и вот тот аргумент с которым была вызвана такая функиция внутри промиса и будет ровняться ошибке 
  ...
  использование промисов подразумевает создание промиса и выполнения определенных действий

  мы можем использовать такую функцию как fetch - она доступна только в браузере 

  с помощью сервиса jsonplaceholder мы можем отправлять запросы на получение данных с удаленного сервера и удаленный сервер будет нам отвечать в
  формате json 

  получается следующее, когда мы отправляем fetch запрос, fetch возвращает промис, т.е создается промис, который будет в состоянии panding, далее
  мы должны каким то образом обработать результать промиса т.е когда удаленый сервер нам ответит, мы должны взять ответ и получить из него данные(массив
    или массив объектов), пока промис в состоянии panding функции которые стоят в круглых скобках не будут вызваны, но как только промис поменяет 
    свое состояние(либо исполнен, либо отклонен), тогда вызовется одна из колбэк функций ( resolve или reject), то есть если сервер успешно вернул ответ 
    т.е промис был исполнен, то вызовется вот эта колбэк функция (response => response.json()) и этой функции передастся ответ от сервера 
    Так вот мы получаем ответ от сервера (response), присваеваем этот ответ как значение параметру response этой колбэк функции и потом мы должны 
    вызвать например метод json  в этом объекте response для того что бы получить данные ответа от сервера, что интересно json метод также возвращает
    промис и из за того что json возвращает промис, мы добавляем еще один вызов then. в итоге этой (json => console.log(json)) колбэек функции для 
    параметра json будет объект, либо массив объектов 
    в конце стоит метод catch на случай, если один из предыдудущих промисов вернул ошибку 
...
Даю очень хороший пример, которым я объясняю новичкам,
Объект - это один из наших машин ( трактор, самосвал, грузовик, подъемник)
У трактора есть - методы - это его возможности, функционал (например поднимать ковш, вскапывать землю)
У техниеи есть кнопки, руль итп - ими управляем техникой - Вызов метода - это нажать на кнопку (запустить двигатель, подять ковш, включить фары итп)
Значение типа String (или другие значения) - это например - взять ковшом землю, взять ковшом - щебень. 

Очень банальный пример:
console.log('hello word)  -  мы выведем в консоль логи (информацию техническую) с надписью hello word
трактор.поднятьковш('земля') - условно мы поднимем ковш трактора с землей

Суть объектов - в том, что языком они изначально прописаны, есть некий базовый набор этих объектов и их методов - их мы учим.

Суть программирования на JS - писать свои объекты и методы, описывая взаимосвязь за счет алгоритмов.

Условно, мы можем создать такой объект, как дом. house
У дома будут открывать двери в квартиры. doorOpen
В квартиру мы можем положить put данные или взять get  (например, мы храним дни рождения в квартирах)

Получается,
В объекте ДОМ открыть квартиру # 313 и взять дату рождения
house.doorOpen.get(313)